...
export default function OfertasTienda() {
  ...
  }, [busqueda, categoriaSeleccionada, ordenPrecio, ofertasOriginales]);
  ...
  const cargarOfertas = async () => {
    try {
      ...
      setOfertasOriginales(ofertasEnriquecidas);
      setOfertasFiltradas(ofertasEnriquecidas);
    } catch (error) {
      ...
      <div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-2 md:p-3 rounded-2xl mb-8 shadow-lg flex flex-col md:flex-row gap-3">
          <div className="relative flex-1 flex items-center">
            ...
            <InputText
              placeholder="Buscar oferta por título o desarrollador..."
              value={busqueda}
        ...
        {ofertasOriginales.length === 0 ? (
          ...
          <div className="text-center py-20 border border-slate-800 rounded-2xl bg-slate-900/50">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">No se encontraron ofertas</h2>
            <p className="text-slate-400">Prueba con otro término o elimina los filtros.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {ofertasFiltradas.map(renderCard)}
          </div>
        )}
      </div>
    </div>
