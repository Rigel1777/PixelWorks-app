...
export default function Tienda() {
  ...
  const cargarCatalogo = async () => {
    try {
      setLoading(true);
      // Reutilizamos el servicio que tu compañero ya configuró
      ...
      <div className="p-5">
        ...
        <div className="mt-5 pt-4 border-t border-slate-800">
          <Button 
            ...
            className="w-full bg-sky-600 hover:bg-sky-500 border-none"
            onClick={() => console.log("Añadir", juego.id)}
          />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-400 mt-2">Encuentra tu próxima aventura en PixelWorks.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {juegos.map(renderCard)}
      </div>
    </div>
  );
}