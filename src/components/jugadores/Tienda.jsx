...
export default function Tienda() {
  ...
  useEffect(() => {
    ...
    if (categoriaSeleccionada) {
      resultado = resultado.filter(
        (juego) => juego.categoriaId === categoriaSeleccionada,
      );
    }
    ...
    if (desarrolladorSeleccionado) {
      resultado = resultado.filter(
        (juego) => juego.desarrolladorId === desarrolladorSeleccionado,
      );
    }
  ...
  const cargarCatalogo = async () => {
    try {
      ...
      setDesarrolladores(dataDesarrolladores);

      const juegosEnriquecidos = dataJuegos.map((juego) => {
        const cat = dataCategorias.find(
          (c) => c.id === juego.categoriaId,
        );

        const des = dataDesarrolladores.find(
          (d) => d.id === juego.desarrolladorId,
        );
  ...
  const opcionesOrden = [
    {
      label: "Del más bajo al más alto",
      value: "asc",
    },
    {
      label: "Del más alto al más bajo",
      value: "desc",
    },
  ];
                ...
                {juego.precioConDescuento ? (
                  <div className="flex flex-col">
                    ...
                    <span className="text-green-400 font-bold text-lg">
                      $
                      {Number(juego.precioConDescuento).toFixed(
                        2,
                      )}
                    </span>
                  </div>
                ) : (
                  <span className="text-sky-400 font-bold text-lg block">
                    ${Number(juego.precio).toFixed(2)}
                  </span>
                )}
              </div>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10">
      ...
      <div>
        <h2 className="text-2xl font-black text-white border-l-4 border-sky-500 pl-3 mb-6">
          Explorar Catálogo
        </h2>

        {/* FILTROS */}
        <div className="bg-slate-900 border border-slate-800 p-3 rounded-2xl mb-8 shadow-lg">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
            ...
            <Dropdown
              ...
              showClear
              className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl flex items-center hover:border-slate-700 transition-colors shadow-inner min-h-[48px]"
            />
            ...
            <Dropdown
              ...
              optionValue="id"
              onChange={(e) =>
                setDesarrolladorSeleccionado(e.value)
              }
              placeholder="Todas las desarrolladoras"
              filter
              filterPlaceholder="Buscar desarrolladora..."
              showClear
              className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl flex items-center hover:border-slate-700 transition-colors shadow-inner min-h-[48px]"
            />
